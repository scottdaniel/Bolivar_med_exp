...
# 2020-04-15 Fix issues with mapping file (no need to run)
...
Therefore, both manual and automatic fix has been performed.
...
Mapping_MS_Expo <- do.call("rbind", lapply(list(Mapping_import$VL, Work_VR15_BL, Work_VR16_BL), FUN = function(x){x[Mapping_common_header]})) %>% filter(Village != "Kadansakaduinha", !is.na(Age))
Mapping_MS_Expo$Ethnicity[which(Mapping_MS_Expo$Ethnicity=="Yekwana")] <- "YEKWANA"
saveRDS(Mapping_MS_Expo, file = "../data/Mapping_MS_Expo.Rds")
```
...
saveRDS(DM_uu, file = "../data/DM_uu_all_20180530.Rds")

# Weighted unifrac
DM_wu <- beta_import$weighted_unifrac %>% as.matrix() %>% as.dist()
saveRDS(DM_wu, file = "../data/DM_wu_all_20180530.Rds")

DM_bc <- beta_import$bray_curtis %>% as.matrix() %>% as.dist()
saveRDS(DM_bc, file = "../data/DM_bc_all_20200604.Rds")
```

# Decoide
## run decoide
```{r}
load("output/Mapping_MS_Expo.Rdata")
Yrs <- c("2015", "2016")
...
for (BB in BodySites) {
    ...
    for (YY in Yrs){
        # YY = "2015"
        Work_dat <- Mapping_MS_Expo %>% filter(Body_Site==BB, Year==YY)
        write.table(Work_dat, file = paste0("output/Mapping_MS_Expo_", BB, "_", YY, ".txt"), sep = "\t", quote = F, row.names = F)
    }
}
```
...
ord_qzas <- lapply(list.files(path = path1, pattern = "Mapping_MS_Expo.*ord.qza", full.names = T), read_qza)

names(ord_qzas) <- list.files(path = path1, pattern = "Mapping_MS_Expo.*ord.qza") %>% gsub(pattern = "Mapping_MS_Expo_|_ord.qza", replacement = "", .)

dist_qzas <- lapply(list.files(path = path1, pattern = "Mapping_MS_Expo.*dist.qza", full.names = T), read_qza)
names(dist_qzas) <- list.files(path = path1, pattern = "Mapping_MS_Expo.*dist.qza") %>% gsub(pattern = "Mapping_MS_Expo_|_dist.qza", replacement = "", .)

saveRDS(ord_qzas, file = "../data/Ord_deicode_all_20200706.Rds")
saveRDS(dist_qzas, file = "../data/DM_deicode_all_20200604.Rds")
...
```{r, eval = FALSE}
# Biom table
dat <- read_qza(file = paste0(path_ana, "/4-Div_dada/rarefied_table.qza"))# bddf0151-4eac-4835-9213-2867f2c70c74
biom <- dat$data %>% as.matrix() %>% as.data.frame()
saveRDS(biom, file = "../data/FT_rarefied_full_11142019.Rds")
```
