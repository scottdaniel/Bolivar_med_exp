...
p <- ggplot(Work_dat, aes(x = Age_num, y = get(AA), color = Urban, shape = Gender)) +
    ...
    facet_grid(.~Year) +
    theme_jw(base_size = 8) + theme(aspect.ratio = 0.8, panel.background = element_rect(color = "black")) +
    labs(x = "Age (Years)", y = AA, title = BB)
print(p)
set_panel_size(p, file = paste0("../output/figures/", AA, "_", BB, ".pdf"), width = unit(3, "in"), height = unit(2.4, "in"), panel.size = T, useDingbats = F)
