...
for (bb in BodySites){
  for (dd in Indices){
    for(yy in Yrs){
      dat_new_ = dat_new %>% 
        dplyr::select(any_of(models[[bb]][[dd]][[yy]]$model %>% colnames())) %>%
        distinct()
      dat_pred[[bb]][[dd]][[yy]] = cbind(dat_new_, 
                                         predict(models[[bb]][[dd]][[yy]], newdata = dat_new_, interval = "confidence"))
...
for (dd in Indices){
  ...
  for (bb in BodySites){
    ...
    ggplot(dat_, aes_string(x = "Age_num", y = dd, color = "Urban", shape = "Gender")) +
      ...
      theme_jw(base_size = 8) + theme(aspect.ratio = 0.7, panel.background = element_rect(color = "black"), plot.title = element_text(hjust = 0), plot.title.position = "plot", legend.key.width = unit(1,"cm")) +
      labs(color = "Exposure") +
      #guides(shape = guide_legend(override.aes = list(alpha = 1, size = 1.5))) +
      ggtitle(bb)
    ggsave(filename = "tmp.pdf", width = 3.5)
