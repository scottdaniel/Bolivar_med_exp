...
for (bb in BodySites){
    for (aa in Indices){
        Work_dat <- Work %>% dplyr::filter(Body_Site==bb, SampleGroup=="Villagers", Age_num>=1)
...
for (bb in BodySites){
    for (aa in Indices){
        for (yy in Yrs){
            Work_dat <- Work %>% dplyr::filter(Body_Site==bb, Year==yy, SampleGroup=="Villagers", Age_num>=1, Age_num<18)
...
ggplot(lm.stats.tbl.2 %>% dplyr::filter(!Indice %in% c("observed_otus", "shannon")), aes(x = Factors, y = p_value, color = Body_Site)) +
    geom_point() +
...
ggplot(lm.stats.tbl.2 %>% dplyr::filter(!Indice %in% c("observed_otus", "shannon")), aes(x = Factors, y = Eta2_partial, color = Body_Site)) +
    geom_point() +
    geom_path(aes(group = Year, linetype = Year)) +
...
for (bb in BodySites){
    for (aa in Indices){
        ...
        ggplot(dat_, aes_string(x = "Age_grp2", y = aa, color = "Year")) +
            geom_point(size = 1.5, position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.7)) +
            geom_boxplot(aes(fill = Year), width = 0.5, color = "black", position = position_dodge(width = 0.7), outlier.alpha = 0) +
            stat_pvalue_manual(dat_.test, label = "p.adj", tip.length = 0, size = 8/.pt) +
            facet_grid(.~Expo) +
            theme_jw() + theme(aspect.ratio = 0.8) +
