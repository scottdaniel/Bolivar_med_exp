...
#------- Create working overall dataset  ---------## 
...
write.table(lm.stats.tbl, file = "../results/tables/alpha_lm_stats_expo.txt", sep = "\t", row.names = F, quote = F)
...
for (bb in BodySites){
    for (aa in Indices){
        for (yy in Yrs){
            Work_dat <- dat_alpha %>% dplyr::filter(Body_Site==bb, Year==yy, SampleGroup=="Villagers", Age_num>=1) %>%
                mutate(ExpoC = ifelse(Expo=="Low", -0.5, 0.5), GenderC = ifelse(Gender=="M", -0.5, 0.5)) # recoding for the contrast
            # Obtain the full linear model
            fit.full <- lm(paste0(aa, " ~ Age_num + ExpoC + GenderC"), data = Work_dat)
            # Obtain the null model with no predictors, this is necessary for forward selection use stepAIC
            fit.null <- lm(paste0(aa, " ~ 1"), data = Work_dat)
            
            # Forward selection, scope parameters provides the full model
            regforward.out <- stepAIC(fit.null, direction = "forward", trace = 4, scope = ~ Age_num + ExpoC + GenderC)
            
            lm.model <- lm(regforward.out$terms, data = Work_dat)

            if ("Age_num" %in% names(lm.model$coefficients)){
...
for (bb in BodySites){
    for (aa in Indices){
        for (yy in Yrs){
            ...
            if(length(fit_$coefficients)==1){
                next
            }
            fit_anova = models[[bb]][[aa]][[yy]] %>% Anova(., type = 3) 
            fit_omega = omega_squared(fit_anova, ci = 0.95) 
            if("segmented" %in% class(fit_)){
                 # effect size estimates
                fit_estimate = as.data.frame(summary(fit_)$Ttable)["Estimate"] %>% rownames_to_column(var = "Factor")
                fit_stats = merge(fit_estimate, fit_anova, by.x =1, by.y = 0, all = T) %>% merge(., fit_omega, by = 1, all = T) %>% rename(Coefficient = Estimate) %>% filter(Factor != "psi1.Age_num")
                fit_stats[which(fit_stats$Factor=="U1.Age_num"), ]$`Pr(>F)` = NA
            } else {
                fit_estimate = as.data.frame(parameters::model_parameters(fit_)) %>% select(Parameter, Coefficient)
                fit_stats = merge(fit_estimate, fit_anova, by.x = 1, by.y = 0, all = T) %>% merge(., fit_omega, by = 1, all = T) %>% rename(Factor = Parameter)

            }
            fit_stats_final[[paste(bb, aa, yy, sep = "-")]] = fit_stats %>% rename(p_value = `Pr(>F)`) %>% mutate(Body_Site = bb, Indice = aa, Year = yy)
        }
    }
}
fit_stats_final.tbl = data.table::rbindlist(fit_stats_final, use.names = T, fill = T) %>% arrange(Body_Site, Indice, Year, Factor) %>% mutate(Omega2_partial = ifelse(is.na(Omega2), Omega2_partial, Omega2), Omega2 = NULL)
write.table(fit_stats_final.tbl, file = "../results/tables/alpha_fit_stats_final_expo.txt", sep = "\t", row.names = F, quote = F)

cols_to_extract = c("Coefficient", "p_value", "Omega2_out")
spec <- tibble(
    .name = rep(Indices, 3) %>% sort() %>% paste(., c("Coefficient", "p_value", "Omega2_out"), sep = "_"),
    .value = rep(cols_to_extract, 4),
    Indice = rep(Indices, 3) %>% sort()
)
fit_stats_final.tbl %>% filter(!is.na(Omega2_partial)) %>% complete(Body_Site, Year, Factor) %>%
    mutate(
        Coefficient = ifelse(Indice %in% c("faith_pd", "observed_otus"), round(Coefficient, 2), round(Coefficient, 3)), 
        p_value = ifelse(p_value<0.001, "<0.001", as.character(round(p_value, 3))), 
        Omega2_partial = round(Omega2_partial, 3),
        CI_low = round(CI_low, 3),
        CI_high = round(CI_high, 3),
        Omega2_out = paste0(Omega2_partial, "(", CI_low, ", ", CI_high, ")"),
        Factor = factor(Factor, levels = c("ExpoC", "Age_num", "U1.Age_num", "GenderC"))
        ) %>% pivot_wider_spec(id_cols = c(Factor, Body_Site, Year), spec) %>% arrange(Body_Site, Year, Factor) %>% write_clip(object_type = "table")
```
#### Graphs
```{r}
Indices <- colnames(alpha[c(2:5)])
...
## Effects of year
### Analysis specific parameters loading
```{r}
Indices <- colnames(alpha[c(2:5)])
BodySites <- c("Feces", "Mouth", "Nose", "Right_Arm", "Right_Hand")
Expos = c("Low", "Medium")
...
### Aanlysis by age groups
```{r}
stats_test <- list()
for (bb in BodySites){
    for (aa in Indices){
        #break
        dat_ = dat_alpha %>% dplyr::filter(Body_Site==bb, SampleGroup=="Villagers", Age_num>=1) %>% mutate(Year = as.character(Year), Age_grp2 = factor(Age_grp2, levels = c("Age_0-3", "Age_3-8", "Age_8-18", "Adults")))
        dat_.test = dat_ %>% group_by(Expo, Age_grp2) %>% wilcox_test(formula(paste0(aa, "~ Year"))) %>% add_xy_position(x = "Age_grp2", dodge = 0.7) %>% ungroup() %>% mutate(p.adj = p.adjust(p, method = "fdr") %>% round(digits = 3), p.adj.signif = ifelse(p.adj<=0.1, p.adj, "ns"), Body_Site = bb) %>% merge(., dat_ %>% group_by(Expo, Age_grp2, Year) %>% summarise(Mean = mean(get(aa))) %>% summarise(Diff = Mean[Year==2016]-Mean[Year==2015], Diff_pct = Diff/Mean[Year==2015]), by = c("Expo", "Age_grp2"))
        stats_test[[paste(bb, aa, sep = "-")]] <- dat_.test

        p <- ggplot(dat_, aes_string(x = "Age_grp2", y = aa, color = "Year")) +
            geom_boxplot(aes(fill = Year), width = 0.5, color = "black", position = position_dodge(width = 0.7), size = 0.2, outlier.alpha = 0, alpha = 0.5) +
            ...
            labs(x = "") + ggtitle(bb)
        print(p)
        set_panel_size(p = last_plot(), file = paste0( "../results/figures/explore/alpha_compare_year_", aa, "_", bb, ".pdf"), width =  unit(3, "in"), height = unit(2.4, "in"))
    }
}

stats_test.tbl = data.table::rbindlist(stats_test, use.names = T, fill = T)
stats_test.tbl %>% mutate(Out = paste0(round(Diff_pct*100, 2), "%(", p.adj, ")")) %>% pivot_wider(id_cols = c(Body_Site, .y.), names_from = c(Expo, Age_grp2), values_from = Out) %>% write_clip(object_type = "table")
```

### Modeling with Year as a factor
#### Building full multiple linear regression models with interaction terms
```{r, eval=FALSE}
alpha_full_lm_models = list()
lm.stats.tbl = list()
for (bb in BodySites){
    for (aa in Indices){
        for (ee in Expos){
            Work_dat <- dat_alpha %>% dplyr::filter(Body_Site==bb, Expo == ee , SampleGroup=="Villagers", Age_num>=1) %>% mutate(Year = factor(Year))
            lm.model <- lm(paste0(aa, " ~ Age_num + Year + Gender + Age_num:Year + Age_num:Gender +Year:Gender"), data = Work_dat, contrasts = list(Year = "contr.sum", Gender = "contr.sum"))
            lm.stats = Anova(lm.model, type = 3) %>% # Type 3 ss
                merge(., effectsize::eta_squared(., ci = 0.95), by.x = 0, by.y = 1, all = T) %>% # effect size estimates
                rename(Factors = Row.names) %>% mutate(Expo = ee, Body_Site = bb, Indice = aa)
            alpha_full_lm_models[[bb]][[aa]][[ee]] = lm.model
            lm.stats.tbl[[paste(bb, aa, ee, sep = "-")]] = lm.stats
        }
    }
}
lm.stats.tbl = do.call("rbind", lm.stats.tbl)
# full model (multiple linear regression) statistics for alpha diversity in all age
write.table(lm.stats.tbl, file = "../results/tables/alpha_lm_stats_year.txt", sep = "\t", row.names = F, quote = F)

lm.stats.tbl.2 = lm.stats.tbl %>% dplyr::filter(!is.na(Eta2_partial)) %>% rename(p_value = `Pr(>F)`) %>% mutate(Factors = factor(Factors, levels = c("Age_num", "Gender", "Year", "Age_num:Year", "Age_num:Gender", "Year:Gender"))) %>% arrange(Body_Site, Indice, Expo, Factors)
    
ggplot(lm.stats.tbl.2, aes(x = Factors, y = p_value, color = Body_Site)) +
    geom_point() +
    geom_path(aes(group = Expo, linetype = Expo)) +
    facet_grid(Body_Site~Indice) +
    scale_y_continuous(breaks = c(0, 0.05, 0.1, 0.2, 1.0)) +
    coord_cartesian(ylim = c(0, 0.2)) +
    theme_jw() + theme(aspect.ratio = 0.6, axis.text.x = element_text(angle = 90, hjust = 1)) +
...
for (bb in BodySites){
    for (aa in Indices){
        for (ee in Expos){
            Work_dat <- dat_alpha %>% dplyr::filter(Body_Site==bb, Expo==ee, SampleGroup=="Villagers", Age_num>=1) %>%
                mutate(YearC = ifelse(Year==2015, -0.5, 0.5), GenderC = ifelse(Gender=="M", -0.5, 0.5)) # recoding for the contrast
            # Obtain the full linear model
            fit.full <- lm(paste0(aa, " ~ Age_num + YearC + GenderC"), data = Work_dat)
            # Obtain the null model with no predictors, this is necessary for forward selection use stepAIC
            fit.null <- lm(paste0(aa, " ~ 1"), data = Work_dat)
            
            # Forward selection, scope parameters provides the full model
            regforward.out <- stepAIC(fit.null, direction = "forward", trace = 4, scope = ~ Age_num + YearC + GenderC)
            ...
            qqnorm(fit.final$residuals)
            ff = (fit.final$terms %>% as.character())[3]
            models[[bb]][[aa]][[ee]] = fit.final
            if ("segmented" %in% class(fit.final)){
                model_tbls = rbind(model_tbls, cbind(bb, aa, ee, ff, "segmented"))
            } else {
                model_tbls = rbind(model_tbls, cbind(bb, aa, ee, ff, "linear"))
            }
        }
    }
}

colnames(model_tbls) = c("Body_Site", "Indice", "Expo", "Model", "Model.type")
model_tbls$Model <- gsub(" + psi1.Age_num", "", model_tbls$Model, fixed = T) %>% paste0(model_tbls$Indice, " ~ ", .)
...
for (bb in BodySites){
    for (aa in Indices){
        for (ee in Expos){
            fit_ = models[[bb]][[aa]][[ee]]
            if(length(fit_$coefficients)==1){
                next
            }
            fit_anova = models[[bb]][[aa]][[ee]] %>% Anova(., type = 3) 
            fit_omega = omega_squared(fit_anova, ci = 0.95) 
            if("segmented" %in% class(fit_)){
                 # effect size estimates
                fit_estimate = as.data.frame(summary(fit_)$Ttable)["Estimate"] %>% rownames_to_column(var = "Factor")
            ...
            fit_stats_final[[paste(bb, aa, ee, sep = "-")]] = fit_stats %>% rename(p_value = `Pr(>F)`) %>% mutate(Body_Site = bb, Indice = aa, Expo = ee)
        }
    }
}
fit_stats_final.tbl = data.table::rbindlist(fit_stats_final, use.names = T, fill = T) %>% arrange(Body_Site, Indice, Expo, Factor) %>% mutate(Omega2_partial = ifelse(is.na(Omega2), Omega2_partial, Omega2), Omega2 = NULL)
write.table(fit_stats_final.tbl, file = "../results/tables/alpha_fit_stats_final_year.txt", sep = "\t", row.names = F, quote = F)
...
fit_stats_final.tbl %>% filter(!is.na(Omega2_partial)) %>% complete(Body_Site, Expo, Factor) %>%
    mutate(
        Coefficient = ifelse(Indice %in% c("faith_pd", "observed_otus"), round(Coefficient, 2), round(Coefficient, 3)), 
        p_value = ifelse(p_value<0.001, "<0.001", as.character(round(p_value, 3))), 
        Omega2_partial = round(Omega2_partial, 3),
        ...
        CI_high = round(CI_high, 3),
        Omega2_out = paste0(Omega2_partial, "(", CI_low, ", ", CI_high, ")"),
        Factor = factor(Factor, levels = c("YearC", "Age_num", "U1.Age_num", "GenderC"))
        ) %>% pivot_wider_spec(id_cols = c(Factor, Body_Site, Expo), spec) %>% arrange(Body_Site, Expo, Factor) %>% write_clip(object_type = "table")
```
...
dat_new = complete(cbind(Age_num = seq(1, 60, 1), 
                         Year = c(2015, 2016), 
                         Gender = c("F", "M")) %>% 
                       as.data.frame(), Age_num, Year, Gender) %>% 
    mutate(Age_num = as.numeric(Age_num), YearC = ifelse(Year==2015, -0.5, 0.5), GenderC = ifelse(Gender=="M", -0.5, 0.5))

dat_pred = list()
for (bb in BodySites){
    for (aa in Indices){
        for(ee in Expos){
            dat_pred[[bb]][[aa]][[ee]] = cbind(dat_new, predict(models[[bb]][[aa]][[ee]], newdata = dat_new, interval = "confidence"))
...
for (ee in Expos){
    for (aa in Indices){
        ...
        for (bb in BodySites){
            #bb = "Feces"
            dat_ = dat_alpha %>% dplyr::filter(Body_Site==bb, Expo==ee, SampleGroup=="Villagers", Age_num>=1)  %>% mutate(Year = factor(Year))
            dat_pred_ = dat_pred[[bb]][[aa]][[ee]]
            mod = model_tbls %>% dplyr::filter(Body_Site==bb, Expo==ee, Indice==aa) %>% pull(Model)
            p.yr = (models[[bb]][[aa]][[ee]] %>% car::Anova(type = "3") %>% as.data.frame())["YearC", "Pr(>F)"]
            p.yr = ifelse(p.yr<0.001, "P.Year<0.001", paste0("P.Year=", round(p.yr, 3)))
            p = ggplot(dat_, aes_string(x = "Age_num", y = aa, color = "Year", shape = "Gender")) +
                geom_point(size = 1.5, alpha = 0.6) +
                geom_line(data = dat_pred_, aes(y = fit, linetype = Gender), size = 1) +
                geom_ribbon(data = dat_pred_, aes(y = fit, ymin = lwr, ymax = upr, fill = Year, alpha = Gender), color = NA, show.legend = F) +
                annotate("label", x = Inf, y = Inf, label = paste(stringi::stri_wrap(c(mod, p.yr),width = 50), collapse = "\n"), hjust = 1, vjust = 1, size = 9/.pt) +
                scale_color_manual(values = c("#80b1d3", "#fb8072"), limits = force) +
                scale_fill_manual(values = c("#80b1d3", "#fb8072"), limits = force) +
                scale_shape_manual(values = c(16, 1)) +
                scale_linetype_manual(values = c(1, 2)) +
                scale_alpha_manual(values = c(0.4, 0.2)) +
                theme_jw(base_size = 14) + theme(aspect.ratio = 0.7, panel.background = element_rect(color = "black"), plot.title = element_text(hjust = 0, vjust = 1), plot.title.position = "panel", legend.key.width = unit(1,"cm"), legend.margin = margin(rep(-3, 4))) +
        ...
        p_f = p_list[["Feces"]] + p_list[["Mouth"]] + guide_area() + p_list[["Nose"]] + p_list[["Right_Arm"]] + p_list[["Right_Hand"]] + plot_layout(guides = "collect", nrow = 2) + plot_annotation(title = ee, tag_levels = 'A') & theme(plot.tag = element_text(size = 10, face = "bold"))
        print(p_f)
        set_panel_size(p = p_f, g = patchworkGrob(p_f), file = paste0( "../results/figures/explore/alpha_regression_", aa, "_", ee, ".pdf"), width =  unit(3, "in"), height = unit(2.1, "in"))
    }
}
```
...
for (bb in BodySites){
    for (aa in Indices){
        dat_ = dat_alpha %>% dplyr::filter(Body_Site==bb, SampleGroup=="Villagers", Age_num>=1) %>% mutate(Year = as.character(Year), Age_grp2 = factor(Age_grp2, levels = c("Age_0-3", "Age_3-8", "Age_8-18", "Adults")), Expo = as.character(Expo))
        ...
        print(p_list[[bb]][[aa]])
        set_panel_size(p = p_list[[bb]][[aa]], file = paste0("../results/figures/explore/delta_alpha_expo_", aa, "_", bb, ".pdf"), width = unit(3, "in"), height = unit(2.4, "in"))
    }
}
...
for (bb in BodySites){
    for (aa in Indices){
        ...
        ggplot(d_alpha, aes(x = Age_grp2, y = delta_alpha)) +
            ...
            theme_jw(base_size = 8) + theme(aspect.ratio = 0.8) +
            labs(y = paste0("Absolute value \u0394(", aa, ")")) + ggtitle(bb)
        set_panel_size(file = paste0("../results/figures/explore/delta_alpha_year_", aa, "_", bb, ".pdf"), width = unit(3, "in"), height = unit(2.4, "in"))
    }
}

d_alpha_stats_test <- list()
for (bb in BodySites){
    for (aa in Indices){
        dat_ = dat_alpha %>% dplyr::filter(Body_Site==bb, SampleGroup=="Villagers", Age_num>=1) %>% mutate(Year = as.character(Year), Age_grp2 = factor(Age_grp2, levels = c("Age_0-3", "Age_3-8", "Age_8-18", "Adults")), Expo = as.character(Expo))
        d_alpha = split(dat_, list(dat_$Year, dat_$Age_grp2, dat_$Expo)) %>%
            lapply(., function(x){dist(x[[aa]], method = "manhattan") %>% as.numeric}) %>% do.call("c", .) %>% data.frame(delta_alpha = .) %>% rownames_to_column(var = "Tmp") %>% separate(Tmp, c("Year", "Age_grp2", "Expo"), sep = "\\.") %>% mutate(Expo = NULL, Age_grp2 = factor(Age_grp2, levels = c("Age_0-3", "Age_3-8", "Age_8-18", "Adults")))
        
        d_alpha.mean = d_alpha %>% group_by(Year, Age_grp2) %>% summarise(Mean = mean(delta_alpha)) %>% ungroup %>% split(., list(.$Year, .$Age_grp2)) %>% lapply(., function(x){pull(x, Mean)})
        d_alpha.test = d_alpha %>% group_by(Year) %>% wilcox_test(delta_alpha ~ Age_grp2) %>% add_xy_position(x = "Age_grp2") %>% ungroup() %>% mutate(p.adj = p.adjust(p, method = "fdr") %>% round(digits = 3), p.adj.signif = ifelse(p.adj<=0.1, p.adj, "ns"), Body_Site = bb, .y. = aa) %>% ungroup() %>% rowwise() %>% mutate(Diff = d_alpha.mean[[paste(Year, group2, sep = ".")]] - d_alpha.mean[[paste(Year, group1, sep = ".")]])
        
        d_alpha_stats_test[[paste(bb, aa, sep = "-")]] <- d_alpha.test
        next
        
        p_list[[bb]][[aa]] <- ggplot(d_alpha, aes(x = Age_grp2, y = delta_alpha)) +
            geom_quasirandom(size = 0.5, alpha = 0.2) +
            geom_boxplot(width = 0.5, fill = NA, size = 0.3, outlier.alpha = 0) +
            stat_pvalue_manual(d_alpha.test, label = "p.adj", tip.length = 0, size = 8/.pt) +
            facet_grid(.~Year) +
            scale_color_manual(values = c("black", "black")) +
            theme_jw(base_size = 8) + theme(aspect.ratio = 0.8) +
...
d_alpha_stats_test.tbl = data.table::rbindlist(d_alpha_stats_test, use.names = T, fill = T)
