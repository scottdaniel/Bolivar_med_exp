...
for (bb in BodySites){
    for (aa in Indices){
        for (yy in Yrs){
            if (length(models[[bb]][[aa]][[yy]]$coefficients)==1){
                next
            } else {
                fit_stats = models[[bb]][[aa]][[yy]] %>% Anova(., type = 3) %>% merge(., effectsize::eta_squared(., ci = 0.95), by.x = 0, by.y = 1, all = T) %>% # effect size estimates
                    rename(Factors = Row.names) %>% mutate(Year = yy, Body_Site = bb, Indice = aa) %>% print
            }
