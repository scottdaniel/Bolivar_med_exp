...
for (YY in Yrs){
  ...
  for (BB in BodySites){
    ...
    for (DD in Indices){
        ...
        adonis2(DM_tmp ~ Age_grp2 + Expo + Gender, by = "margin", data = Mf %>% filter(SampleID %in% attr(DM_tmp, "Labels")))
        
        rda_fit <- dbrda(DM_tmp ~ Expo + Gender + Age_grp2, data = Mf %>% filter(SampleID %in% attr(DM_tmp, "Labels")))
        # proportions of axes
        eigenvals(rda_fit)/sum(eigenvals(rda_fit))*100
