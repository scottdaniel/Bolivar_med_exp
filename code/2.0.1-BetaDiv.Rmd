...
for (YY in Yrs){
  ...
  for (BB in BodySites){
    ...
    for (DD in Indices){
        ...
        stats_disp_perm <- lapply(stats_disp, function(x){permutest(x)$tab$`Pr(>F)`[1]}) %>% do.call("rbind", .) %>% as.data.frame %>% rownames_to_column(var = "Age_grp2") %>% rename(p.disp = V1) %>% mutate(Variable = "Expo")
        
        stats_tbl <- lapply(stats, calcOmega2_dm) %>% data.table::rbindlist(., use.names = T, idcol = "Age_grp2") %>% mutate(Body_Site = BB, Year = YY, Index = DD) %>% merge(., stats_disp_perm, by = c("Age_grp2", "Variable"), all.x = T)
        
        stats_tbl2 <- stats_tbl %>% mutate(Omega2_partial =round(Omega2_partial, 3), stat_label = paste0(Variable, ": p = ", `Pr(>F)`, "; Omega2 = ", Omega2_partial), stat_label = ifelse(Variable=="Expo", paste0(stat_label, "; p.disp = ", p.disp), stat_label)) %>% group_by(Age_grp2) %>% summarise(stat_label = paste(stat_label[2], stat_label[1], sep = "\n")) 
        
        beta_stats[[paste0(YY, BB, DD)]] <- stats_tbl
        
        pcoa_tmp <- parallel::mclapply(dms, function(x){cmdscale(d = x, k = 3, list. = T, eig = T)}, mc.cores = nc)
        p_lst <- list()
        for (f in names(pcoa_tmp)){
            varPC1 <- round(pcoa_tmp[[f]]$eig[1]/sum(pcoa_tmp[[f]]$eig)*100, 2)
            varPC2 <- round(pcoa_tmp[[f]]$eig[2]/sum(pcoa_tmp[[f]]$eig)*100, 2)
            varPC3 <- round(pcoa_tmp[[f]]$eig[3]/sum(pcoa_tmp[[f]]$eig)*100, 2)
            plot_dt <- pcoa_tmp[[f]]$points[,1:3] %>% merge(., Mf, by.x = 0, by.y = 1, all.x = T)
            p_lst[[f]] <- ggplot(plot_dt, aes(x = V1, y = V2, color = Expo)) +
                geom_point(size = 1, aes(shape = Gender)) +
                stat_ellipse() +
                geom_label(data = stats_tbl2 %>% filter(Age_grp2==f), x = -Inf, y = Inf, aes(label = stat_label), hjust = 0, vjust = 1, inherit.aes = F, size = 8/.pt) +
                facet_grid(.~Age_grp2) +
                theme_jw(base_size = 14) + theme(aspect.ratio = 1, panel.background = element_rect(colour = "black"), panel.grid.major = element_line(colour="grey90",linetype="dashed", size=0.2), legend.direction = "vertical", strip.text = element_text(face="bold", color = "white", size = rel(1))) +
        ...
        print(p)
        set_panel_size(p = p, g = patchworkGrob(p), file = paste0("../results/figures/explore/pcoa_pc1v2_", DD, "_", BB, "_", YY, ".pdf"), width = unit(3, "in"), height = unit(3, "in"), useDingbats = F)
    }
  }
}

beta_stats_tbl <- do.call("rbind", beta_stats) %>% arrange(Index, Age_grp2, Variable, Body_Site, Year) %>% select(Index, Age_grp2, Variable, Body_Site, Year, everything()) %>% filter(!is.na(`F`))
write.table(beta_stats_tbl, file = "../results/tables/beta_stats_by_age.txt", sep = "\t", row.names = F)


write_clip(beta_stats_tbl %>%  pivot_wider(id_cols = c(Index, Year, Body_Site, Variable), names_from = Age_grp2, values_from = c(Omega2_partial, `Pr(>F)`, p.disp)) %>% arrange(Index, Year, Body_Site, Variable))

```

### dbRDA with regular beta diversity index
#### By different years
```{r}
Indices = c("uu", "wu", "bc")
Yrs <- c("2015", "2016")
...
for (YY in Yrs){
  #YY = "2015"
    for (DD in Indices){
        ...
        # Main effect plot
        p_list[["Main"]][[BB]] <- ggplot(rda_dat_center1, aes(x = dbRDA1, y = dbRDA2)) + 
        ...
        print(p_list[["Main"]][[BB]])
        #set_panel_size(p_list[["Main"]][[BB]], file = paste("../results/figures/explore/dbrda", DD, BB, YY, "main.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
        ...
        # Interaction plot
        p_list[["Inter"]][[BB]] <- ggplot(rda_dat_2, aes(x = dbRDA1, y = dbRDA2)) + 
            ...
            geom_label_repel(data = rda_dat_center2, aes(label = Age_grp2), size = 7/.pt, point.padding = 5) +
            geom_hline(yintercept = 0, linetype = "dashed", size = 0.2) +
            geom_vline(xintercept = 0, linetype = "dashed", size = 0.2) +
            theme_jw(base_size = 10) + theme(aspect.ratio = 1, panel.background = element_rect(color = "black")) +
            ...
            guides(color = "none") +
            labs(x = paste0("dbRDA1 (", var_dbrda1, "%)"), y = paste0("dbRDA2 (", var_dbrda2, "%)"), title = paste(BB, DD, YY))
        print(p_list[["Inter"]][[BB]])
        #set_panel_size(p_list[["Inter"]][[BB]], file = paste("../results/figures/explore/dbrda", DD, BB, YY, "ExpoAge.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
        ...
        stats_tbl2 <- stats_tbl %>% mutate(Omega2_partial =round(Omega2_partial, 3), stat_label = paste0(Variable, ": p = ", `Pr(>F)`, "; Omega2 = ", Omega2_partial))
        
        beta_stats[[paste0(YY, BB, DD)]] <- stats_tbl
        }
        p1 <- p_list[["Main"]][["Feces"]] + p_list[["Main"]][["Mouth"]] + guide_area() + p_list[["Main"]][["Nose"]] + p_list[["Main"]][["Right_Arm"]] + p_list[["Main"]][["Right_Hand"]] + plot_layout(guides = "collect", nrow = 2) + plot_annotation(title = paste(YY, DD), tag_levels = 'A') & theme(plot.tag = element_text(size = 10, face = "bold"))
        set_panel_size(g = patchworkGrob(p1), file = paste("../results/figures/explore/dbrda_combine", DD,  YY, "Main.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
        
        p2 = p_list[["Inter"]][["Feces"]] + p_list[["Inter"]][["Mouth"]] + guide_area() + p_list[["Inter"]][["Nose"]] + p_list[["Inter"]][["Right_Arm"]] + p_list[["Inter"]][["Right_Hand"]] + plot_layout(guides = "collect", nrow = 2) + plot_annotation(title = paste(YY, DD), tag_levels = 'A') & theme(plot.tag = element_text(size = 10, face = "bold"))
        set_panel_size(g = patchworkGrob(p2), file = paste("../results/figures/explore/dbrda_combine", DD,  YY, "Inter.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
    }
}
...
for (DD in Indices){
    ...
    for (BB in BodySites){
        ...
        Mf <- Mapping_work %>% filter(Body_Site==BB, SampleGroup=="Villagers")
        # distance matrix for YY, BB, and DD
        DM_tmp <- subset_dm_by_names(get(paste0("DM_", DD)), Mf$SampleID)
        Mf <- Mf %>% filter(SampleID %in% attr(DM_tmp, "Labels")) %>% mutate(Year = factor(Year))
        
        # Adonis
        stats <- adonis2(DM_tmp ~ Expo + Gender + Age_grp2 + Year, by = "margin", data = Mf)
        stats_tbl <- calcOmega2_dm(stats) %>% mutate(Body_Site = BB, Index = DD)
        stats_tbl2 <- stats_tbl %>% mutate(Omega2_partial =round(Omega2_partial, 3), stat_label = paste0(Variable, ": p = ", `Pr(>F)`, "; Omega2 = ", Omega2_partial))
        
        beta_stats[[paste0(BB, DD)]] <- stats_tbl
        
        rda_fit <- dbrda(DM_tmp ~ Expo + Gender + Age_grp2 + Year, data = Mf)
        ...
        rda_axes <- scores(rda_fit, scaling = "sites", choices = seq(5))$sites %>% as.data.frame() # vectors projecting original data onto axis k, using scaling type 2
        rda_dat <- merge(rda_axes, Mf, by.x = 0, by.y = 1)
        rda_dat_center1 <- lapply(c("Expo", "Gender", "Age_grp2", "Year"), function(x){rda_dat %>% group_by_at(x) %>% summarise(across(starts_with("dbRDA"), mean)) %>% rename(Lvls = !!(x))}) 
        names(rda_dat_center1) <- c("Expo", "Gender", "Age_grp2", "Year")
        ...
        p_list[["Main"]][[BB]] <- ggplot(rda_dat_center1, aes(x = dbRDA1, y = dbRDA2)) + 
            ...
            guides(fill = "none") +
            labs(x = paste0("dbRDA1 (", var_dbrda1, "%)"), y = paste0("dbRDA2 (", var_dbrda2, "%)"), title = paste(BB, DD))
        print(p_list[["Main"]][[BB]])
        set_panel_size(p_list[["Main"]][[BB]], file = paste("../results/figures/explore/dbrda", DD, BB, "main.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
        
        rda_dat_center2 <- rda_dat %>% group_by(Expo, Year, Age_grp2) %>% summarise(across(starts_with("dbRDA"), mean))
        rda_dat_2 <- merge(rda_dat, rda_dat_center2, by = c("Expo", "Year", "Age_grp2"), suffixes = c("", ".c"))
        # Interaction plot
        p_list[["Inter"]][[BB]] <- ggplot(rda_dat_2, aes(x = dbRDA1, y = dbRDA2)) + 
            geom_segment(aes(x = dbRDA1.c, y = dbRDA2.c, xend = dbRDA1, yend = dbRDA2, color = Expo), size = 0.3) +
            geom_point(aes(fill = Expo, shape = Year), size = 1, alpha = 0.5) +
            geom_point(data = rda_dat_center2, size = 5, aes(fill = Expo, shape = Year, alpha = Year)) +
            geom_label_repel(data = rda_dat_center2, aes(label = Age_grp2), size = 7/.pt, point.padding = 5) +
            geom_hline(yintercept = 0, linetype = "dashed", size = 0.2) +
            geom_vline(xintercept = 0, linetype = "dashed", size = 0.2) +
            theme_jw(base_size = 10) + theme(aspect.ratio = 1, panel.background = element_rect(color = "black")) +
            ...
            labs(x = paste0("dbRDA1 (", var_dbrda1, "%)"), y = paste0("dbRDA2 (", var_dbrda2, "%)"), title = paste(BB, DD))
        print(p_list[["Inter"]][[BB]])
        set_panel_size(p_list[["Inter"]][[BB]], file = paste("../results/figures/explore/dbrda", DD, BB,"ExpoAge.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
    }
    p1 <- p_list[["Main"]][["Feces"]] + p_list[["Main"]][["Mouth"]] + guide_area() + p_list[["Main"]][["Nose"]] + p_list[["Main"]][["Right_Arm"]] + p_list[["Main"]][["Right_Hand"]] + plot_layout(guides = "collect", nrow = 2) + plot_annotation(title = paste(YY, DD), tag_levels = 'A') & theme(plot.tag = element_text(size = 10, face = "bold"))
    set_panel_size(g = patchworkGrob(p1), file = paste("../results/figures/explore/dbrda_combine", DD,  YY, "Main.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
...
for (YY in Yrs){
    for (BB in BodySites){
        Mf <- Mapping_work %>% filter(Year==YY, Body_Site==BB, SampleGroup=="Villagers")
        Ord <- Ord_dc[[paste(BB, YY, sep = "_")]]$data
        
        Ord_dat <- merge(Ord$Vectors, Mf, by = "SampleID")
        Ord_dat_center2 <- Ord_dat %>% group_by(Expo, Age_grp2) %>% summarise(across(c(PC1, PC2, PC3), mean))
        Ord_dat_2 <- merge(Ord_dat, Ord_dat_center2, by = c("Expo", "Age_grp2"), suffixes = c("", ".c"))
        
        var_ord1 <- round(Ord$ProportionExplained$PC1*100, 1) 
        var_ord2 <- round(Ord$ProportionExplained$PC2*100, 1)
        
        ggplot(Ord_dat_2, aes(x = PC1, y = PC2)) + 
            geom_segment(aes(x = PC1.c, y = PC2.c, xend = PC1, yend = PC2, color = Expo), size = 0.3) +
            geom_point(aes(fill = Expo), size = 1, shape = 21, alpha = 0.5) +
            geom_point(data = Ord_dat_center2, size = 5, aes(fill = Expo), shape = 21) +
            geom_label_repel(data = Ord_dat_center2, aes(label = Age_grp2), size = 7/.pt, point.padding = 5) +
            geom_hline(yintercept = 0, linetype = "dashed", size = 0.2) +
            geom_vline(xintercept = 0, linetype = "dashed", size = 0.2) +
            theme_jw(base_size = 10) + theme(aspect.ratio = 1, panel.background = element_rect(color = "black")) +
            scale_fill_manual(values = Expo_color, limits = force) +
            scale_color_manual(values = Expo_color) +
            guides(color = "none") +
            labs(x = paste0("PC1 (", var_ord1, "%)"), y = paste0("PC2 (", var_ord2, "%)"))
    }
}
```
