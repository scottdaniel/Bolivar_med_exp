...
for (YY in Yrs){
  ...
  for (BB in BodySites){
    ...
    for (DD in Indices){
        ...
        # Main effect plot
        p1 <- ggplot(rda_dat_center1, aes(x = dbRDA1, y = dbRDA2)) + 
            ...
            guides(fill = "none") +
            labs(x = paste0("dbRDA1 (", var_dbrda1, "%)"), y = paste0("dbRDA2 (", var_dbrda2, "%)"), title = paste("Main Effect Plot", BB, DD, YY))
        print(p1)
        set_panel_size(p1, file = paste("../results/figures/explore/dbrda", DD, BB, YY, "main.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
        ...
        # Interaction plot
        p2 <- ggplot(rda_dat_2, aes(x = dbRDA1, y = dbRDA2)) + 
            ...
            guides(color = "none") +
            labs(x = paste0("dbRDA1 (", var_dbrda1, "%)"), y = paste0("dbRDA2 (", var_dbrda2, "%)"), title = paste("Interaction Plot", BB, DD, YY))
        print(p2)
        set_panel_size(p2, file = paste("../results/figures/explore/dbrda", DD, BB, YY, "ExpoAge.pdf", sep = "_"), width = unit(3.5, "in"), height = unit(3.5, "in"))
...
beta_stats_tbl <- data.table::rbindlist(beta_stats) %>% arrange(Index, Variable, Body_Site, Year) %>% select(Index, Variable, Body_Site, Year, everything()) %>% filter(!is.na(`F`))
