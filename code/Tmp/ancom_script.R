...
Ancom_mf_lst <- Mapping_work2 %>% filter(SampleGroup=="Villagers", Body_Site %in% BodySites) %>% group_by(Year, Body_Site, Age_grp2) %>% group_split()

Ancom_rsl <- parallel::mclapply(Ancom_mf_lst, function(x){
    list(x[1, c("Year", "Body_Site", "Age_grp2")], ancom_wrap(ft_glom$Genus %>% select(Taxa, any_of(x$SampleID)), x, adjusted = T, main.var = "Urban", adj.formula = "Gender", repeat.var = NULL, longitudinal = F, random.formula = NULL, multcorr = 2, sig = 0.05, prev.cut = 1))
    }, mc.cores = 20)

save(Ancom_rsl, file = "output/Ancom_rsl.Rdata")
