...
filter_low_coverage <- function(props, frac_cutoff=0.4, min_ab=0){
  frac_nonzero <- function (x) mean(x > min_ab) #since the mean of a logical vector is equal to the fraction of nonzero's
  apply(props, 1, frac_nonzero) >= frac_cutoff
}

filter_rare_taxa <- function(props, prop_cut) {
  rows_to_keep <- apply(props, 1, max) >= prop_cut
  return(props[rows_to_keep,])
}
...
heatmap_grouped <- function(summed_props, heatmap_s, grps = c("study_group", "study_week"), fname=NULL, thre=0.8, option=1, prop_cut=0.01, satu_limit=0.4, taxa = NULL, ...){
  ...
  if (!is.null(taxa)) {
    rownames(heatmap_props) <- taxa[rownames(heatmap_props)]
  }
