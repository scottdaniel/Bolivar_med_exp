...
for (yy in Yrs){
    for (aa in Indices){
        ...
        for (bb in BodySites){
            #bb = "Feces"
            dat_ = dat_alpha %>% dplyr::filter(Body_Site==bb, Year==yy, SampleGroup=="Villagers", Age_num>=1)
            dat_pred_ = dat_pred[[bb]][[aa]][[yy]]
            #mod = model_tbls %>% dplyr::filter(Body_Site==bb, Year==yy, Indice==aa) %>% pull(Model)
            (models[[bb]][[aa]][[yy]] %>% car::Anova(type = "3") %>% as.data.frame())["Expo", "Pr(>F)"] %>% 
            p = ggplot(dat_, aes_string(x = "Age_num", y = aa, color = "Expo", shape = "Gender")) +
                ...
                annotate("label", x = Inf, y = Inf, label = str_wrap(paste0(aa, " ~ ", mod), width = 50), hjust = 1, vjust = 1, size = 12/.pt) +
